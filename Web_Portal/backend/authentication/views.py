...
class LoginView(APIView):
    ...
    def post(self, request):
        print(f"DEBUG: Headers: {request.headers.get('X-Is-Mobile')}")
        ...
        print(f"Login attempt: user={user.username}, mobile={is_mobile}, has_device={device is not None}")
...
class Setup2FAView(APIView):
    ...
    def get(self, request):
        # Generate a new device (unverified)
        # Check if one exists first?
        # Let's assume we create a new one for 'Setup' flow
        user = request.user
        
        # In a real app we wouldn't save immediately, or we save as inactive
        # Let's checking if user already has an active one?
        ...
        # Check if an inactive device already exists
        device = TOTPDevice.objects.filter(user=user, is_active=False).first()
        
        if device:
            secret = device.secret_key
        else:
            # Generate new secret only if no inactive device exists
...
class VerifyOTPView(APIView):
    # This view should be accessible with the 'temp_token'
    # For now, let's assume the frontend sends the user ID or we use the temp token as a normal bearer
    # If using normal bearer, 'request.user' will be populated.
    permission_classes = [permissions.IsAuthenticated] 
...
class DeviceListView(APIView):
    ...
    def get(self, request):
        devices = TOTPDevice.objects.filter(user=request.user, is_active=True)
...
class InitiatePushAuthView(APIView):
    ...
    def post(self, request):
        ...
        # In a real app, you'd get IP and Browser from request.META
...
class PendingPushAuthView(APIView):
    ...
    def get(self, request):
        # Get latest pending request for mobile app
